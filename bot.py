...
async def server(websocket, path):
    while True:
        ...
        #to acces the specific websocket connection later
        ...
        if session.query(User).filter_by(token = message["token"]).first() is None:
            ...
            session.commit()
        #sending received message to the bot(to specific support) with inline button
...
async def support_msg(message: types.Message):
    ...
    #getting a specific websocket connection
